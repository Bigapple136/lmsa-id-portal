...
## 12. Fix: Notification "Mark as read" Never Stuck and Bell Counter Never Cleared; Missing Clear/Delete (commit pending — not yet committed)
...
| *(pending)* | Fix notification per-admin read state (service-role `auth.uid()` bug); add clear/delete endpoints + UI | Not committed |
