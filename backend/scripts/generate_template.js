...
async function generateTemplate() {
  ...
  const ALL_COLS = ['student_id', 'full_name', 'year_level', 'position', 'programme', 'blood_type', 'student_email', 'emergency_contact_name', 'emergency_contact_phone'];
  const COL_META = {
  ...
  FORM_ROWS.push([
    { t: 's', v: 'LMSA Student ID Card Portal' },
    null, null, null, null,
  ]);

  // Row 1: subtitle (merged across A-E)
  FORM_ROWS.push([
    { t: 's', v: 'Single Student Entry Form  \u2192  Fill in below, then use the Developer tab to add buttons.' },
    null, null, null, null,
  ]);
  ...
  FORM_ROWS.push([
    { t: 's', v: '\u26A0  Macros must be enabled. Enable Developer tab > Visual Basic > paste VBA code. See Instructions sheet.' },
    null, null, null, null,
  ]);
  ...
  const studentsData = [
    ALL_COLS,
    ['AMD-2024-0001', 'Josephine K. Freeman', '3rd Year', 'Class Representative', 'MBBS', 'O+', 'josephine@email.com', 'Mary Freeman', '+231 770 000000'],
  ...
  // Header row styles
  const QR_KEYS = ['programme', 'blood_type', 'student_email', 'emergency_contact_name', 'emergency_contact_phone'];
  ALL_COLS.forEach((key, i) => {
  ...
  sample.forEach((_, i) => {
    ...
    cell.s = {
      italic: true, fontColor: { rgb: '888780' },
      fill: { patternType: 'solid', fgColor: { rgb: 'F3F4F6' } },
    };
  });
...
End Sub`;
  ...
  const instrData = [
    ...
    ['Step 5', 'Add the Buttons (optional but recommended)', '',
  ...
  instrWs['!cols'] = [
    { wch: 28 }, { wch: 30 }, { wch: 20 }, { wch: 90 }
  ];
  instrWs['!rows'] = instrData.map((_, i) => {
    if (i === 0) return { hpt: 30 };
    if (['SECTION 1: HOW TO SET UP THE MACROS', 'SECTION 2: HOW TO USE THE FORM',
         'SECTION 3: COLUMN REFERENCE', 'SECTION 4: VBA SOURCE CODE', 'SECTION 5: COLUMN REFERENCE'].includes(instrData[i][0])) return { hpt: 22 };
