  ...
  return (
    <div style={{ display:'flex', gap:'16px', alignItems:'flex-start', flexWrap:'wrap' }}>
      ...
      <div style={{ flexShrink:0 }}>
        ...
        >
          {templateUrl
            ? <img src={templateUrl} alt="template" style={{ width:'100%', height:'100%', objectFit:'fill', display:'block', pointerEvents:'none' }}/>
            : <div style={{ width:'100%', height:'100%', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'12px', color:'var(--muted)' }}>No template uploaded</div>
          }
          ...
          {activeFields.map(field => {
            ...
            return (
              ...
              >
                <span style={{ fontSize:'8px', fontWeight:'700', color, whiteSpace:'nowrap', pointerEvents:'none' }}>
                  {label}
                </span>
              </div>
            )
        ...
        <div style={{ marginTop:'8px', display:'flex', flexWrap:'wrap', gap:'5px' }}>
          {activeFields.map(f => (
            <div key={f} style={{ display:'flex', alignItems:'center', gap:'4px', cursor:'pointer' }} onClick={() => setSelected(f)}>
              <div style={{ width:'8px', height:'8px', borderRadius:'2px', background: FIELD_META[f].color }}/>
      ...
      <div style={{ flex:1, minWidth:'180px' }}>
        ...
        {sel && selMeta ? (
          <div style={{ background:'var(--bg)', border:'0.5px solid var(--border)', borderRadius:'8px', padding:'12px', marginBottom:'12px' }}>
            <div style={{ fontSize:'12px', fontWeight:'500', color:'var(--text)', marginBottom:'10px', display:'flex', alignItems:'center', gap:'6px' }}>
              <div style={{ width:'10px', height:'10px', borderRadius:'2px', background:selMeta.color, flexShrink:0 }}/>
              {selMeta.label}
            </div>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'8px' }}>
              ...
              {sel.type === 'text' && (<>
                ...
                <div style={{ gridColumn:'1/-1', display:'flex', alignItems:'center', gap:'8px' }}>
                  ...
                  <label htmlFor={`bold-${selected}`} style={{ fontSize:'12px', color:'var(--text)', cursor:'pointer' }}>Bold</label>
                </div>
              </>)}
            </div>
          </div>
        ) : (
          <div style={{ fontSize:'12px', color:'var(--hint)', padding:'12px', background:'var(--bg)', border:'0.5px solid var(--border)', borderRadius:'8px', marginBottom:'12px', lineHeight:'1.6' }}>
            Click a colored box on the card to select it, then adjust its position, size, and text style here.
          </div>
        )}
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:'8px' }}>
          ...
          <button className="btn-outline" onClick={() => { setLayout({ ...DEFAULT_LAYOUT }); setSelected(null) }} style={{ width:'100%', fontSize:'12px' }}>
            Reset to defaults
          </button>
          {msg && <div className={msg.ok ? 'success-box' : 'error-box'} style={{ fontSize:'12px' }}>{msg.text}</div>}
        </div>
      </div>
    </div>
  )
